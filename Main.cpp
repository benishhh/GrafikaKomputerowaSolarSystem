...
int main() {
	...
	Texture texturesPlanks[]{
		Texture("nwm.png", "diffuse", 0, GL_RGBA, GL_UNSIGNED_BYTE),
		Texture("nwm.png", "specular", 1, GL_RED, GL_UNSIGNED_BYTE)
	};
	Texture texturesSun[]{
	...
	std::vector<Vertex> sunVertices = GenerateSphereVertices((69634*wielkosc)/2, 50, 50); // 6.69 to œrednica
	...
	std::vector<Vertex> uranVertices = GenerateSphereVertices(57070 * wielkosc, 50, 50);
	...
	glm::vec3 uranPos = glm::vec3(2050.0 / 2 * odleg³oœæ, 0.0f, 0.0f); // gdzie siê znajduje na scenie
	glm::mat4 uranModel = glm::mat4(1.0f);
	uranModel = glm::translate(uranModel, uranPos);
	...
	glm::vec3 neptunPos = glm::vec3(2700.0/2 * odleg³oœæ, 0.0f, 0.0f); // gdzie siê znajduje na scenie
